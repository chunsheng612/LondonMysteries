...
# Pattern for the WHOLE generateLevels and any trailing garbage before the next valid function
# We know the next valid function is generateExtendedStoryLevels or something similar
# But the file is so messy now. I'll just find the start of generateLevels and replace a large chunk.

start_marker = 'generateLevels() {'
# Let's find the next "MagicAlchemyLab" method or something
# Actually, I'll just look for the next "    " followed by a word and "() {"

matches = list(re.finditer(r'^    \w+\(.*\)\s*\{', content, re.MULTILINE))
gen_levels_idx = -1
next_func_idx = -1

for i, m in enumerate(matches):
    if 'generateLevels' in m.group():
        gen_levels_idx = i
        if i + 1 < len(matches):
            next_func_idx = i + 1
        break

if gen_levels_idx != -1 and next_func_idx != -1:
    start_pos = matches[gen_levels_idx].start()
    end_pos = matches[next_func_idx].start()
    
    new_code = """    generateLevels() {
        const chapters = [
            {
                key: 'alleys',
                title: '霧都暗巷',
                ...
                orders: [
                    ...
                    { title: '三種證言', client: '瑪麗', request: '房客證詞矛盾。', rule: 'three-types', ruleLabel: '三方證詞', clue: '線索只涉及三個人。', intro: '旅館充滿了流言。', perfect: '真相大白！', good: '鎖定範圍。', rough: '證詞太混亂。', fail: '三個人都在說謊。' },
                    ...
                    { title: '全員追蹤', client: '維克', request: '跨國盜竊案。', rule: 'spectrum', ruleLabel: '全員追蹤', clue: '五格代表五名嫌犯。', intro: '警督盯著我。', perfect: '全員落網！', good: '首腦被抓。', rough: '案子破了但受傷。', fail: '被騙得團團轉。' }
                ]
            },
            {
                key: 'clocktower',
            ...
            },
            {
                ...
                orders: [
                    ...
                    { title: '三方證詞', client: '律師', request: '三位繼承人。', rule: 'three-types', ruleLabel: '遺產紛爭', clue: '只涉及三種特徵級別。', intro: '法庭上的博弈。', perfect: '官司獲勝！', good: '證據不足。', rough: '陷入僵持。', fail: '輸掉訴訟。' },
                    ...
                    { title: '莊園終局', client: '爵士', request: '最後的審判。', rule: 'crown', ruleLabel: '莊園終局', clue: '六格冠式。', intro: '最後的告白。', perfect: '真相大白！', good: '留有遺憾。', rough: '慘勝一局。', fail: '真相湮滅。' }
                ]
            },
            {
                key: 'underground',
                ...
                mentor: '「地底由拳頭決定。」',
                orders: [
                    { title: '黑市暗號', client: '老鼠', request: '首尾接頭暗號。', rule: 'bookend', ruleLabel: '接頭對應', clue: '首尾素材相同級別。', intro: '地底生存法則。', perfect: '人贓俱獲！', good: '身分暴露。', rough: '交易取消。', fail: '沉入泰晤士河。' },
...
"""
    ...
    with open('js/app.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully replaced generateLevels with clean code")
else:
    print(f"Indices not found: gen_levels={gen_levels_idx}, next_func={next_func_idx}")
